...
def wpm(typed, elapsed):
    ...
    #     return 0.
    # str_len = 0
    # for item in type_words:
    #     str_len = str_len + len(item) + 1
    str_len = len(typed)
    if str_len == 0:
        return 0.
    vel = (str_len / 5) * 60 / elapsed
    return vel
    # END PROBLEM 4
...
def shifty_shifts(start, goal, limit):
    ...
    # if len(start) == 0:
    #     return 1 + shifty_shifts(start, goal[1:], limit)
    # if start[0] != goal[0]:
    #     return 1 + shifty_shifts(start[1:], goal[1:], limit)
    # if len(goal) == 0:
    #     return 0
    # if shifty_shifts(start, goal, limit) > limit:
    #     return limit + 1
...
def meowstake_matches(start, goal, limit):
    ...
    if len(start) == 0 or len(goal) == 0:  # Fill in the condition
        ...
        # print("----------")
        "*** YOUR CODE HERE ***"
        # return 1 + min(min(add_diff, substitute_diff), remove_diff))
        # 递归不用自己想具体做法，只需要让他自己找出最小值即可
...
def fastest_words(game):
    ...
    for i in players:
        list.append([])
    for j in words:
        min_time = time(game, 0, j)
        index = 0
        for i in players:
            if time(game, i, j) < min_time:
                min_time = time(game, i, j)
                index = i
        list[index].append(word_at(game, j))
    return list
