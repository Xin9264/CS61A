...
def trade(first, second):
    ...
    equal_prefix = lambda x, y, m, n: sum(x[:m]) == sum(y[:n])
    while not equal_prefix(first, second, m, n) and m < len(first) and n < len(second):
        if sum(first[:m]) < sum(second[:n]):
            m += 1
        else:
            n += 1

    if equal_prefix(first, second, m, n):
...
def make_glookup(class_assignments):
    ...
    all_value = 0
    def cal(name, value):
        nonlocal score, all_value
        score += value
        all_value += class_assignments[name]
        return score/all_value
    return cal
